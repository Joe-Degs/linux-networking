...
KUBERNETES_PUBLIC_IP="$(get_ip gateway)"
# kube-api server
...
cfssl gencert \
  ...
  -ca-key=ca-key.pem \
  -config=ca-config.json \
  -hostname=$(get_ip master0),$(get_ip master1),$(get_ip master2),${KUBERNETES_PUBLIC_IP},127.0.0.1,kubernetes.default \
  -profile=kubernetes \
...
# generate data encryption key and yaml config for encrypting data at rest
